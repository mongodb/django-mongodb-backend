...
from django_mongodb_backend.fields import EncryptedIntegerField
...
class IntegerFieldTests(TestCase):
    ...
    rel_db_type_class = EncryptedIntegerField
        ...
        for callable_limit in (True, False):
            with self.subTest(callable_limit=callable_limit):
                if min_backend_value is not None:
                    ...
                    limit_value = (
                        (lambda value=min_custom_value: value)
                        if callable_limit
                ...
                if max_backend_value is not None:
                    ...
                    limit_value = (
                        (lambda value=max_custom_value: value)
                        if callable_limit
    ...
    def test_invalid_value(self):
        ...
        for exception, value in tests:
            with self.subTest(value):
                msg = f"Field 'value' expected a number but got {value}."
...
class ValidationTests(SimpleTestCase):
    ...
    def test_choices_validation_supports_named_groups(self):
        f = EncryptedIntegerField(choices=(("group", ((10, "A"), (20, "B"))), (30, "C")))
    ...
    def test_nullable_integerfield_raises_error_with_blank_false(self):
        f = EncryptedIntegerField(null=True, blank=False)
        with self.assertRaises(ValidationError):
            f.clean(None, None)

    def test_nullable_integerfield_cleans_none_on_null_and_blank_true(self):
        f = EncryptedIntegerField(null=True, blank=True)
        self.assertIsNone(f.clean(None, None))

    def test_integerfield_raises_error_on_empty_input(self):
        f = EncryptedIntegerField(null=False)
        with self.assertRaises(ValidationError):
            f.clean(None, None)
        with self.assertRaises(ValidationError):
            f.clean("", None)
    ...
    def test_integerfield_validates_zero_against_choices(self):
        f = EncryptedIntegerField(choices=((1, 1),))
        with self.assertRaises(ValidationError):
            f.clean("0", None)

    def test_enum_choices_cleans_valid_string(self):
        f = EncryptedIntegerField(choices=self.Choices)
    ...
    def test_callable_choices(self):
        ...
        f = EncryptedIntegerField(choices=get_choices)
