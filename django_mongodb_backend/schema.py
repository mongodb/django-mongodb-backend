...
class BaseSchemaEditor(BaseDatabaseSchemaEditor):
    ...
    def _get_encrypted_fields(
        ...
        # If it doesn't exist, create an index to guarantee unique keyAltNames.
        ...
        # Get the master key (i.e. KMS credentials). N/A for local KMS.
        if kms_provider == "local":
            master_key = None
        else:
            if "KMS_CREDENTIALS" not in connection.settings_dict:
                raise ImproperlyConfigured(
                    f"DATABASES['{self.connection.alias}'] is missing "
                    f"'KMS_CREDENTIALS' required for KMS '{kms_provider}'."
                )
            if kms_provider not in connection.settings_dict["KMS_CREDENTIALS"]:
                raise ImproperlyConfigured(
                    f"DATABASES['{self.connection.alias}']['KMS_CREDENTIALS'] "
                    f"is missing '{kms_provider}' key."
                )
