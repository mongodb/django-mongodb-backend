...
- Fixed ``makemigrations`` to correctly order ``CreateModel`` operations. If
  embedded models aren't created before models that reference them, ``migrate``
  crashes with ``AttributeError: 'str' object has no attribute '_meta'``.
