...
- Defining indexes and constraints on embedded models using ``unique=True``,
  ``db_index=True``, ``Meta.constraints``, and ``Meta.indexes`` is deprecated
  in favor of using :class:`~django_mongodb_backend.indexes.EmbeddedFieldIndex`
  and :class:`~django_mongodb_backend.constraints.EmbeddedFieldUniqueConstraint`
