...
- Ensure unique indexes for ``string``, ``int``, ``long``, ``double``,
  ``decimal``, ``bool``, and ``date`` fields are being used by the query planner.
...
- Users will need to drop and recreate unique indexes on ``string``, ``int``,
  ``long``, ``double``, ``decimal``, ``bool``, and ``date`` fields in order to
  benefit from the more optimized unique constraint filters.
