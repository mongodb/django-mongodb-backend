...
.. csv-table:: Fields in ``django.db.models``
...
.. csv-table:: Fields in ``django_mongodb_backend.fields``
   :header: "Model Field", "Encrypted version available?"
    ...
    :class:`PolymorphicEmbeddedModelField`, No - it may be implemented in the future.
...
as any parameters to configure the query type.
...
See :ref:`querying-encrypted-fields` for an example.

The supported Django lookups for ``equality`` queries are: :lookup:`exact` and
:lookup:`in`. The supported operators are AND (``&``) and OR (``|``).

The supported lookups for ``range`` queries include those of ``equality``
queries as well as :lookup:`lt`, :lookup:`lte`, :lookup:`gt`, :lookup:`gte`,
and :lookup:`range`.

Due to MongoDB limitations, these fields don't support the ``queries``
argument:
...
entire subdocument, in which case you can't query any of the subdocument's
fields, or you can encrypt only selected fields of the subdocument.
...
.. _encrypted-queryset-limitations:

``QuerySet`` limitations
------------------------

In addition to :ref:`Django MongoDB Backend's QuerySet limitations
<known-issues-querying>`, some ``QuerySet`` methods aren't supported on
encrypted fields. Each unsupported method listed here is followed by a sample
...
- :meth:`~django.db.models.query.QuerySet.in_bulk`: "Encrypted fields can't
...
.. _automatic-encryption-limitations:

Automatic Encryption limitations
--------------------------------

There are also several ``QuerySet`` methods that aren't permitted on any models
that use a database connection with Automatic Encryption, regardless of whether
